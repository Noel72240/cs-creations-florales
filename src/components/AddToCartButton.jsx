...
/**
 * @param {{ id: string, title: string, price: number, imageUrl?: string, path?: string, selectedColor?: string }} product
 * @param {string} [label]
...
export default function AddToCartButton({ product, label = 'Ajouter au panier', className = '' }) {
  ...
  const cartId = selectedColor ? `${product.id}::${selectedColor}` : product.id
