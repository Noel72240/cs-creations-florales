...
export default function ArticleProduct() {
  ...
  return (
    <>
      ...
      <section className="py-6 px-4 sm:py-10" style={{ background: 'var(--beige)' }}>
        <div className="max-w-lg mx-auto">
          <nav className="text-xs mb-5 flex flex-wrap items-center gap-1" style={{ color: 'var(--text-mid)' }}>
            <Link to="/" className="hover:text-mauve transition-colors">
              Accueil
            </Link>
            <span aria-hidden>›</span>
            <Link to={rubrique} className="hover:text-mauve transition-colors">
              {rubriqueLabel}
            </Link>
            <span aria-hidden>›</span>
            <span style={{ color: 'var(--violet)' }}>{article.title}</span>
          </nav>

          <div className="article-product-gallery mb-5">
            {mainPhoto ? (
              <img src={mainPhoto} alt={article.title} className="article-product-gallery__main" />
            ) : null}
            {photos.length > 1 ? (
              <div className="article-catalog-dots mt-3" role="tablist" aria-label="Photos du produit">
                {photos.map((url, i) => (
                  <button
                    key={url}
                    type="button"
          ...
          <h1 className="font-heading text-2xl sm:text-3xl font-medium leading-snug mb-2" style={{ color: 'var(--violet)' }}>
            {article.title}
          </h1>
          <p className="font-refined text-xl font-semibold mb-6" style={{ color: 'var(--mauve)' }}>
            {formatEuro(price)}
          </p>
          ...
          <Link to={rubrique} className="btn-outline text-sm inline-block">
